...
// ✅ IMPORT YOUR CONFIGURED ECHO INSTANCE
...
const HistoryChart: React.FC = () => {
  ...
  const formatDataPoint = useCallback(
    (row: any): ChartPoint => {
      return {
        // Backend usually sends 'created_at', simulation might send 'timestamp'
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Take the last 20 points and reverse them so the oldest is on the left
  ...
  useEffect(() => {
    // Connect to the 'sensors' channel
    const channel = echo.channel('sensors');

    console.log('🔌 Subscribed to sensor updates...');

    channel.listen('.sensor.updated', (event: any) => {
      // Handle different payload structures (sometimes event.data, sometimes direct event)
      const payload = event.data || event;
      console.log('📈 New Data Point:', payload);

      const newPoint = formatDataPoint(payload);

      setData((prevData) => {
        // Append new point, keep only the last 20
      ...
      {!loading && !error && (
        <>
          <ResponsiveContainer width="100%" height={250}>
            ...
            >
              ...
              <YAxis
                ...
                stroke="#10b981"
                fontSize={12}
                label={{ value: 'L/min', angle: 90, position: 'insideRight' }}
              />
              <Tooltip
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* 1. Detailed Pressure */}
            <div className="h-48">
              <p className="text-xs font-bold text-center text-blue-600 mb-2">
            ...
            {/* 2. Detailed Flow */}
            <div className="h-48">
              <p className="text-xs font-bold text-center text-green-600 mb-2">
                Flow Sensors (L/min)
              </p>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data}>
